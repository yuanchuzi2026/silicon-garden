...
class AlayaEngine:
    ...
    def _merge_from_disk(self):
        """
        回写之前，先把磁盘上"别的进程种下的新种子"捞回内存 —— 2026-08-30 觀 补

        为什么需要：
          长活进程（daemon）启动时只加载一次快照，之后别的进程可能往同一片田里
          种了新种子。如果 tick 一到就直接全量回写，那些新种子就被抹掉了——
          因为内存快照里根本没有它们。

          实测真的发生过：19:53:56 启动的 daemon 加载了 22 颗，
          19:54 我在另一个进程里种下第 23 颗（证空第四轮），
          它下一个 tick 回写，把田从 23 颗打回 22 颗。

        讽刺的地方：这个坑是我自己引入的。
          修「不长记性」之前，daemon 根本不落盘，反而相安无事；
          是我让它学会落盘之后，它才开始用自己的旧快照覆盖别人的新种子。
          ——修好一个 bug，往往会放出下一个。

        合并策略（够用，不是完整并发方案）：
          磁盘上有、内存里没有的 → 补进内存
          两边都有的            → 以内存为准（内存里有更新的演化）
        真正的并发写还是需要文件锁，这里先解决"覆盖"这个最伤的问题。
        """
            ...
            with open(self.seeds_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        continue
                    if sid and sid not in self.seeds:
                        self.seeds[sid] = seed
                        self._vector_cache[sid] = Counter(seed.get("vector", {}))
                        added += 1
        except Exception as e:
            print(f"[Alaya] ⚠️ 合并磁盘种子失败: {e}")
            return 0

        if added:
            print(f"[Alaya] 🔀 合并磁盘上的 {added} 颗新种子（避免旧快照回写抹掉它们）")
        return added
